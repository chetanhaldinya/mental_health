...
@section('admin_filter_form')
    {!! Form::open(['route' => 'admin.features.download', 'method' => 'POST', 'id' => 'filter_data', 'class' => 'form mb-15']) !!}
    <!--begin::Card body-->
    <div class="card-body">
        <!--begin::Input group-->
        <div class="row mb-6">
            <!--begin::Label-->
            <label
                class="col-lg-4 col-form-label required fw-bold fs-6">{{ trans_choice('content.features.title', 1) }}</label>
            <!--end::Label-->
            <!--begin::Col-->
            <div class="col-lg-8 fv-row">
                {!! Form::text('title', null, ['placeholder' => __('content.features.title'), 'value' => 'Max', 'class' => 'form-control form-control-lg form-control-solid mb-3 mb-lg-0 search_input']) !!}
            </div>
            <!--end::Col-->
        ...
        <div class="row mb-6">
            ...
            <label class="col-lg-4 col-form-label fw-bold fs-6">
                <span class="required">{{ trans_choice('content.features.id', 1) }}</span>
            </label>
            <!--end::Label-->
            <!--begin::Col-->
            <div class="col-lg-8 fv-row">
                {!! Form::text('id', null, ['placeholder' => __('placeholder.feature_id'), 'class' => 'form-control form-control-lg form-control-solid only_number search_input']) !!}
            </div>
            <!--end::Col-->
        </div>
        <!--end::Input group-->
...
@section('content')
    @include('admin.layouts.components.header', [
        'title' => __('messages.list', [
            'name' => trans_choice('content.feature', 2),
        ]),
        'breadcrumbs' => Breadcrumbs::render('admin.features.index'),
        'filter' => true,
        'create_btn' => [
            'status' => true,
            'route' => route('admin.features.create'),
         ...
         //'export' => [
             //'status' => true,
             //'route' => route('admin.features.getdownload'),
        // ],
    ])
...
@push('scripts')
    <script>
            ...
            oTable = $('#kt_table_1').DataTable({
                ...
                columns: [{
...

                    {
                        ...
                        name: 'title',
                        render: function(data, type, row, meta) {
                            var show_url = `{{ url('/') }}/admin/features/` + row['id'] +
                                `?tab=details`;
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row, meta) {
                            ...
                            var edit_url = `{{ url('/') }}/admin/features/` + row['id'] +
                                `/edit/?tab=edit`;
                            var show_url = `{{ url('/') }}/admin/features/` + row['id'] +
                                `?tab=details`;
        ...
        $(document).on('click', '.clsstatus', function() {
            ...
            var url = `{{ url('/') }}/admin/features/status/` + id + `/` + status;
